...
class Log {
  ...
  public json(_object: object, _kind: string = 'INFO'): void {
    const stringObject: string = JSON.stringify(_object);
    this.addLog(_kind, stringObject);
  }

  public test(_string: string, _kind: string = 'WARN'): void {
    this.addLog(_kind, _string);
  }
